...
type LibraryController struct {
	...
	ReserveChannel chan concurrency.ReservationRequest
}
...
func (c *LibraryController) handleReserveBook() {
	fmt.Print("Enter Book ID to reserve: ")
	bID, _ := c.getIntInput()
	fmt.Print("Enter Member ID: ")
	mID, _ := c.getIntInput()

	request := concurrency.ReservationRequest{
		BookID: bID,
		MemberID: mID,
	}

	fmt.Println("Sending reservation request to queue...")
	c.ReserveChannel <- request
}
